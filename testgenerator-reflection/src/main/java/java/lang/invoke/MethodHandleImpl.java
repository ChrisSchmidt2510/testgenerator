...
/* non-public */ abstract class MethodHandleImpl {
	...
	/**
	 * Create a JVM-level adapter method handle to conform the given method handle
	 * to the similar newType, using only pairwise argument conversions. For each
	 * argument, convert incoming argument to the exact type needed. The argument
	 * conversions allowed are casting, boxing and unboxing, integral widening or
	 * narrowing, and floating point widening or narrowing.
	 * 
	 ...
	 * @param monobox if true, unboxing conversions are assumed to be exactly typed
	 *                (Integer to int only, not long or double)
	 * @return an adapter to the original handle with the desired new type, or the
	 *         original target if the types are already identical or null if the
	 *         adaptation cannot be made
	 */
	 ...
	 * This conversion function will be used as a LF NamedFunction. Return a Class
	 * object if a simple cast is needed. Return void.class if void is involved.
	 */
	...
	private static final class AsVarargsCollector extends DelegatingMethodHandle {
		...
		public MethodHandle asTypeUncached(MethodType newType) {
			...
			try {
				...
				assert (collector.type().parameterCount() == newArity) : "newArity=" + newArity + " but collector="
						+ collector;
		...
		boolean viewAsTypeChecks(MethodType newType, boolean strict) {
			...
			assert (type().lastParameterType().getComponentType()
					.isAssignableFrom(newType.lastParameterType().getComponentType())) : Arrays.asList(this, newType);
			return true;
		}
	}
	...
	static class Lazy {
		...
		static {
			...
			try {
				...
				NF_guardWithCatch = new NamedFunction(MHI.getDeclaredMethod("guardWithCatch", MethodHandle.class,
						Class.class, MethodHandle.class, Object[].class));
				...
				MH_selectAlternative = makeIntrinsic(IMPL_LOOKUP.findStatic(MHI, "selectAlternative", MethodType
						.methodType(MethodHandle.class, boolean.class, MethodHandle.class, MethodHandle.class)),
	...
	static boolean profileBoolean(boolean result, int[] counters) {
		// Profile is int[2] where [0] and [1] correspond to false and true occurrences
		// respectively.
		int idx = result ? 1 : 0;
		try {
	...
	static MethodHandle makeGuardWithTest(MethodHandle test, MethodHandle target, MethodHandle fallback) {
		...
		try {
			if (PROFILE_GWT) {
				...
				mh = (BoundMethodHandle) BoundMethodHandle.speciesData_LLLL().constructor().invokeBasic(type, form,
						(Object) test, (Object) profile(target), (Object) profile(fallback), counts);
			} else {
				mh = (BoundMethodHandle) BoundMethodHandle.speciesData_LLL().constructor().invokeBasic(type, form,
	...
	/**
	 * Block inlining during JIT-compilation of a target method handle if it hasn't
	 * been invoked enough times. Corresponding LambdaForm has @DontInline when
	...
	/**
	 * Constructs reinvoker lambda form which block inlining during JIT-compilation
	 * for a particular method handle
	 */
	...
	/**
	 * Counting method handle. It has 2 states: counting and non-counting. It is in
	 * counting state for the first n invocations and then transitions to
	...
	static class CountingWrapper extends DelegatingMethodHandle {
		...
		boolean countDown() {
			if (count <= 0) {
				// Try to limit number of updates. MethodHandle.updateForm() doesn't guarantee
				// LF update visibility.
				if (isCounting) {
					isCounting = false;
					return true;
				} else {
		...
		static void maybeStopCounting(Object o1) {
			...
			if (wrapper.countDown()) {
				// Reached invocation threshold. Replace counting behavior with a non-counting
				// one.
	 ...
	 * class. argL3 and argL4 are auxiliary method handles: argL3 boxes arguments
	 * and wraps them into Object[] (ValueConversions.array()) and argL4 unboxes
	 * result if necessary (ValueConversions.unbox()).
	 *
	 * Having t8 and t10 passed outside and not hardcoded into a lambda form allows
	 * to share lambda forms among catchException combinators with the same basic
	 * type.
	 */
	...
	static MethodHandle fakeMethodHandleInvoke(MemberName method) {
		...
		if (mh != null)
			return mh;
		MethodType type = MethodType.methodType(Object.class, UnsupportedOperationException.class, MethodHandle.class,
				Object[].class);
	 ...
	 * called from the same class loader and protection domain as hostClass. This is
	 * an expensive no-op unless the method which is called is sensitive to its
	 * caller. A small number of system methods are in this category, including
	 * Class.forName and Method.invoke.
	 */
	...
	private static class BindCaller {
		static MethodHandle bindCaller(MethodHandle mh, Class<?> hostClass) {
			...
			if (hostClass == null || (hostClass.isArray() || hostClass.isPrimitive()
					|| hostClass.getName().startsWith("java.") || hostClass.getName().startsWith("sun."))) {
	...
	private static final class WrappedMember extends DelegatingMethodHandle {
		...
		private WrappedMember(MethodHandle target, MethodType type, MemberName member, boolean isInvokeSpecial,
				Class<?> callerClass) {
			super(type, target);
			this.target = target;
			this.member = member;
	...
	/* non-public */
	enum Intrinsic {
		SELECT_ALTERNATIVE, GUARD_WITH_CATCH, NEW_ARRAY, ARRAY_LOAD, ARRAY_STORE, IDENTITY, ZERO, NONE // no intrinsic
																										// associated
	}

	/**
	 * Mark arbitrary method handle as intrinsic. InvokerBytecodeGenerator uses this
	 * info to produce more efficient bytecode shape.
	 */
	...
	// using Integer len instead of int len and no varargs to avoid bootstrapping
	// problems
	...
	/**
	 * Return a method handle that takes the indicated number of Object arguments
	 * and returns an Object array of them, as if for varargs.
	 */
	 ...
	 * corresponding arguments, and then returns a. The value L is a global constant
	 * (LEFT_ARGS).
	 */
	...
	/**
	 * Return a method handle that takes the indicated number of typed arguments and
	 * returns an array of them. The type argument is the array type.
	 */
