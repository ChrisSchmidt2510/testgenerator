...
class DirectMethodHandle extends MethodHandle {
	...
	// Constructors and factory methods in this class *must* be package scoped or
	// private.
	...
	@ForceInline
	/* non-public */static Object getProxyValue(Object obj) {
		if (obj instanceof ReferenceProxy) {
			return ((ReferenceProxy<?>) obj).getValue();
		} else if (obj instanceof LongProxy) {
			return ((LongProxy) obj).getValue();
		} else if (obj instanceof DoubleProxy) {
			return ((DoubleProxy) obj).getValue();
		} else if (obj instanceof FloatProxy) {
			return ((FloatProxy) obj).getValue();
		} else if (obj instanceof BooleanProxy) {
			return ((BooleanProxy) obj).getValue();
		} else if (obj instanceof IntegerProxy) {
			IntegerProxy proxy = (IntegerProxy) obj;

			if (Byte.TYPE == proxy.getDataType()) {
				return proxy.getByteValue();
			} else if (Character.TYPE == proxy.getDataType()) {
				return proxy.getCharValue();
			} else if (Short.TYPE == proxy.getDataType()) {
		...
		throw newInternalError(obj + "no valid Proxy");
	}

	/* non-public */ @SuppressWarnings("unchecked")
	static void setProxyValue(Object proxy, Object value) {
		if (proxy instanceof ReferenceProxy) {
			((ReferenceProxy<Object>) proxy).setValue(value);
		} else if (proxy instanceof LongProxy) {
			((LongProxy) proxy).setValue((long) value);
		} else if (proxy instanceof DoubleProxy) {
			((DoubleProxy) proxy).setValue((double) value);
		} else if (proxy instanceof FloatProxy) {
			((FloatProxy) proxy).setValue((float) value);
		} else if (proxy instanceof IntegerProxy) {
			((IntegerProxy) proxy).setValue((int) value);
		} else if (proxy instanceof BooleanProxy) {
			((BooleanProxy) proxy).setValue((boolean) value);
		}
	}
	 ...
	 * among all fields with the same basicType and refKind.
	 */
	...
	private static LambdaForm preparedFieldLambdaForm(byte formOp, boolean isVolatile, Class<?> ftype) {
		...
		lform = makePreparedFieldLambdaForm(formOp, isVolatile, ftypeKind(ftype), ClassExtension.isProxy(ftype));
	...
	private static LambdaForm makePreparedFieldLambdaForm(byte formOp, boolean isVolatile, int ftypeKind,
		...
		if (isVolatile)
			ctname += "Volatile";
		String getOrPut = (isGetter || (!isGetter && isProxy) ? "get" : "put");
		String linkerName = (getOrPut + ctname); // getObject, putIntVolatile, etc.
		MethodType linkerType;
		if (isGetter || (!isGetter && isProxy))
		...
		final int LINKER_CALL = nameCursor++;
		final int PROXY_GETVALUE = isProxy && isGetter ? nameCursor++ : -1;
		final int PROXY_SETVALUE = isProxy && !isGetter ? nameCursor++ : -1;
		...
		assert (outArgs.length == (isGetter || (!isGetter && isProxy) ? 3 : 4));
		outArgs[0] = UNSAFE;
		if (isStatic) {
		...
		if (needsCast && isGetter) {
			if (isProxy) {
				names[PROXY_GETVALUE] = new Name(Lazy.NF_getProxyValue, names[LINKER_CALL]);
				names[POST_CAST] = new Name(Lazy.NF_checkCast, names[DMH_THIS], names[PROXY_GETVALUE]);
			} else
		...
		static {
			try {
				NamedFunction nfs[] = {
						...
						NF_checkReceiver = new NamedFunction(
								new MemberName(Interface.class.getDeclaredMethod("checkReceiver", Object.class))),
						NF_getProxyValue = new NamedFunction(
								DirectMethodHandle.class.getDeclaredMethod("getProxyValue", Object.class)),
						NF_setProxyValue = new NamedFunction(DirectMethodHandle.class.getDeclaredMethod("setProxyValue",
