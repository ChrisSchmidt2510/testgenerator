...
/* non-public */ final class MemberName implements Member, Cloneable {
	 ...
	 * {@link Class#getSimpleName}. For a method or field, it is the simple name of
	 * the member. For a constructor, it is always {@code "<init>"}.
	 */
	@Override
	 ...
	 * leading parameter, a reference to declaring class. For static methods, it is
	...
	/**
	 * Return the declared type of this member, which must be a field or type. If it
	 * is a type member, that type itself is returned.
	 */
	...
	/**
	 * Utility method to produce the signature of this member, used within the class
	 * file format to describe its type.
	 */
	...
	/**
	 * Utility method to query if this member is a method handle invocation (invoke
	 * or invokeExact).
	 */
	...
	/**
	 * Utility method to query the modifier flags of this member; returns false if
	 * the member is not a method.
	 */
	...
	/**
	 * Utility method to query whether this member is neither public, private, nor
	 * protected.
	 */
	...
	/**
	 * Utility method to query whether this member is accessible from a given lookup
	 * class.
	 */
	...
	/**
	 * Calls down to the VM to fill in the fields. This method is synchronized to
	 * avoid racing calls.
	 */
	...
	/**
	 * Create a name for the given reflected method. The resulting name will be in a
	 * resolved state.
	 */
	 ...
	 * clazz.isInterface. The end result is to get a fully virtualized version of
	 * the MN. (Note that resolving in the JVM will sometimes devirtualize, changing
	 * REF_invokeVirtual of a final to REF_invokeSpecial, and REF_invokeInterface in
	 * some corner cases to either of the previous two; this transform undoes that
	 * change under the assumption that it occurred.)
	 */
	...
	/**
	 * Create a name for the given reflected constructor. The resulting name will be
	 * in a resolved state.
	 */
	@SuppressWarnings("LeakingThisInConstructor")
	...
	/**
	 * Create a name for the given reflected field. The resulting name will be in a
	 * resolved state.
	 */
	...
	/**
	 * Create a name for the given class. The resulting name will be in a resolved
	 * state.
	 */
	...
	/**
	 * Create a name for a signature-polymorphic invoker. This is a placeholder for
	 * a signature-polymorphic instance (of MH.invokeExact, etc.) that the JVM does
	 * not reify. See comments on {@link MethodHandleNatives#linkMethod}.
	 */
	...
	/**
	 * Get the definition of this member name. This may be in a super-class of the
	...
	/**
	 * Decide if two member names have exactly the same symbolic content. Does not
	 * take into account any actual class members, so even if two member names
	 * resolve to the same actual member, they may be distinct references.
	 */
	...
	/**
	 * Create a field or type name from the given components: Declaring class, name,
	 * type, reference kind. The declaring class may be supplied as null if this is
	 * to be a bare name and type. The resulting name will in an unresolved state.
	 */
	 ...
	 * class, name, type, reference kind. It will be a constructor if and only if
	 * the name is {@code "<init>"}. The declaring class may be supplied as null if
	 * this is to be a bare name and type. The last argument is optional, a boolean
	 * which requests REF_invokeSpecial. The resulting name will in an unresolved
	 * state.
	 */
	...
	/**
	 * Query whether this member name is resolved. A resolved member name is one for
	 * which the JVM has found a method, constructor, field, or type binding
	...
	/**
	 * Produce a string form of this member name. For types, it is simply the type's
	 * own string (as reported by {@code toString}). For fields, it is
	 * {@code "DeclaringClass.name/type"}. For methods and constructors, it is
	 * {@code "DeclaringClass.name(ptype...)rtype"}. If the declaring class is null,
	 * the prefix {@code "DeclaringClass."} is omitted. If the member is unresolved,
	 * a prefix {@code "*."} is prepended.
	 */
	...
	/* non-public */ static class Factory {
		...
		/**
		 * Produce a resolved version of the given member. Super types are searched (for
		 * inherited members) if {@code searchSupers} is true. Access checking is
		 * performed on behalf of the given {@code lookupClass}. If lookup fails or
		 * access is not permitted, null is returned. Otherwise a fresh copy of the
		 * given member is returned, with modifier bits filled in.
		 */
		private MemberName resolve(byte refKind, MemberName ref, Class<?> lookupClass) {
			...
			try {
				...
				//
				// What we care about when resolving a MemberName is consistency between DEFC
				// and PTYPES.
				// We do type alias (TA) checks on DEFC to ensure that. DEFC is not known until
				// the JVM
				// finishes the resolution, so do TA checks right after MHN.resolve() is over.
				//
				// All parameters passed by a caller are checked against MH type (PTYPES) on
				// every invocation,
				// so it is safe to call a MH from any context.
				//
				// REFC view on PTYPES doesn't matter, since it is used only as a starting point
				// for resolution and doesn't
		 ...
		 * inherited members) if {@code searchSupers} is true. Access checking is
		 * performed on behalf of the given {@code lookupClass}. If lookup fails or
		 * access is not permitted, a {@linkplain ReflectiveOperationException} is
		 * thrown. Otherwise a fresh copy of the given member is returned, with modifier
		 * bits filled in.
		 */
		 ...
		 * inherited members) if {@code searchSupers} is true. Access checking is
		 * performed on behalf of the given {@code lookupClass}. If lookup fails or
		 * access is not permitted, return null. Otherwise a fresh copy of the given
		 * member is returned, with modifier bits filled in.
		 */
		...
		/**
		 * Return a list of all methods defined by the given class. Super types are
		 * searched (for inherited members) if {@code searchSupers} is true. Access
		...
		/**
		 * Return a list of matching methods defined by the given class. Super types are
		 * searched (for inherited members) if {@code searchSupers} is true. Returned
		 * methods will match the name (if not null) and the type (if not null). Access
		 * checking is performed on behalf of the given {@code lookupClass}.
		 * Inaccessible members are not added to the last.
		 */
		...
		/**
		 * Return a list of all constructors defined by the given class. Access checking
		 * is performed on behalf of the given {@code lookupClass}. Inaccessible members
		 * are not added to the last.
		 */
		...
		/**
		 * Return a list of all fields defined by the given class. Super types are
		 * searched (for inherited members) if {@code searchSupers} is true. Access
		 * checking is performed on behalf of the given {@code lookupClass}.
		 * Inaccessible members are not added to the last.
		 */
		 ...
		 * Return a list of all fields defined by the given class. Super types are
		 * searched (for inherited members) if {@code searchSupers} is true. Returned
		 * fields will match the name (if not null) and the type (if not null). Access
		 * checking is performed on behalf of the given {@code lookupClass}.
		 * Inaccessible members are not added to the last.
		 */
		...
		/**
		 * Return a list of all nested types defined by the given class. Super types are
		 * searched (for inherited members) if {@code searchSupers} is true. Access
		 * checking is performed on behalf of the given {@code lookupClass}.
		 * Inaccessible members are not added to the last.
		 */
